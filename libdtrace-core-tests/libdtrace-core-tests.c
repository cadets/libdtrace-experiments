...
ATF_TC_BODY(DIF_SUBR_NTOHL, tc)
{
	...
	instr = DIF_INSTR_CALL(DIF_SUBR_NTOHL, 3);
	err = dtrace_emul_instruction(instr, estate, mstate, vstate, state);

	ATF_CHECK_EQ(0, err);
#if BYTE_ORDER == LITTLE_ENDIAN
	ATF_CHECK_EQ(0x563412, estate->dtes_regs[3]);
#else
	ATF_CHECK_EQ(0x12345600, estate->dtes_regs[3]);
#endif
...
ATF_TC_BODY(DIF_SUBR_NTOHLL, tc)
{
	...
	instr = DIF_INSTR_CALL(DIF_SUBR_NTOHLL, 3);
	err = dtrace_emul_instruction(instr, estate, mstate, vstate, state);

	ATF_CHECK_EQ(0, err);
#if BYTE_ORDER == LITTLE_ENDIAN
	ATF_CHECK_EQ(0x9A78563412, estate->dtes_regs[3]);
#else
	ATF_CHECK_EQ(0x123456789A000000, estate->dtes_regs[3]);
#endif
...
ATF_TC_BODY(DIF_SUBR_BASENAME, tc)
{
	...
	instr = DIF_INSTR_CALL(DIF_SUBR_BASENAME, 3);
	err = dtrace_emul_instruction(instr, estate, mstate, vstate, state);

	ATF_CHECK_EQ(0, err);
	ATF_CHECK_STREQ("baz", (const char *)estate->dtes_regs[3]);
...
ATF_TC_BODY(DIF_SUBR_CLEANPATH, tc)
{
	...
	instr = DIF_INSTR_CALL(DIF_SUBR_CLEANPATH, 3);
	err = dtrace_emul_instruction(instr, estate, mstate, vstate, state);

	ATF_CHECK_EQ(0, err);
	ATF_CHECK_STREQ("/test/foo/bar/baz", (const char *)estate->dtes_regs[3]);
